...
//!    淡入淡出/双语台词(点击推进)。
//!
//! 图像装载:CG/精灵路径经 `PacFileIndex::read_image_bytes` /
//! `read_cg_bytes`(se 型 stored PNG;扩展名/立绘模糊解析)。
//! 台词:fontdue + 系统 CJK 字体(msjh.ttc)渲染到纹理层。
//!
...
mod audio;
mod scenario;
use scenario::{ScenarioHost, ScenarioPlayer};
...
const SC_BG: u64 = 0x5C_0000_0001;
const SC_FADE: u64 = 0x5C_0000_0002;
const SC_TEXT: u64 = 0x5C_0000_0003;
const SC_WIN: u64 = 0x5C_0000_0004;
...
struct PlayerCore {
    ...
    /// 光标逻辑坐标(@133/@138 注入)。
    cursor_logical: (i64, i64),
    key_pulse: bool,
    key_pulse_frames: u32,
    font: fontdue::Font,
    ...
    sprite_fades: Vec<(u64, Instant, u64, bool)>,
    /// 音频包索引 + 播放器(P9.1)。
    audio_packs: Arc<PacFileIndex>,
    audio: crate::audio::Audio,
    /// 活动选择肢(文本 + 命中区)。
    choices: Option<Vec<(String, [f32; 4])>>,
    /// 最近 CursorMoved 原始像素(winit 0.30 无 cursor_position 轮询)。
    last_cursor_px: Option<(f64, f64)>,
    frame_clicked: bool,
    #[allow(dead_code)]
...
impl Player {
    ...
    fn inject_key(&mut self, key: &str) {
