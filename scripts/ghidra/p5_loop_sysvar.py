# -*- coding: utf-8 -*-
"""P5.1 —— LOOP 家族处理器反编译(0x37 LOOP/0x38 LOOPBREAK/0x39 LOOPCONT/0x3a LOOPEND):
定性 @48(系统变量 0x30)是否为当前 LOOP 迭代计数(读自 obj+0x244 嵌套栈顶记录 +0x10)。"""
import os
...
with pyghidra.open_program(BINARY, project_location=PROJECT_LOC,
    ...
    for addr_val, tag in [(0x00445A00, "CMD_LOOP_0x37"),
                          (0x00445B9C, "CMD_loopbreak_0x38"),
                          (0x00445C54, "CMD_loopcont_0x39"),
                          (0x00445CE8, "CMD_loopend_0x3a"),
                          (0x004428C0, "CMD_gosub_0x2b")]:
