...
- **H2(Likely)字符 vs 字节语义**:若引擎把 POS/LENGTH 当**字节偏移**(SJIS
  步进只用于边界对齐),则 POS=31/LENGTH=5 对 35 字节串完全合法(区间
  [30,35)),我们的字符序数解读把合法请求误判越界。
...
1. **补上下文**:POS 变体错误日志补打 `s{sid} pc={pc} 串字节 ops`(LENGTH
   分支已有,POS 分支缺)——一次运行即可钉实全部失败组。
